...
@section('title')
    {{__('site.add_new_account')}}
@endsection
@section('content')
    @include('provider.layouts.nav')
    <div class="page-contant mt-4">
        <div class="container-fluid overflow-hidden">
            <h5 class="titlePages"> {{__('site.add_new_account')}}</h5>
            <div class="add-form fadedown2">
                <form class="row flex-column align-items-center add-account-form" method="POST" enctype="multipart/form-data" action="{{url('provider/add-account')}}">
                    @csrf
                    <div class="form_label col-md-9 col-12">
                        <input type="text" class="input_focus addofferinput" name="account_name" required/>
                        <label class="float_label">{{__('site.account_name')}}</label>
                    </div>
                    <div class="form_label col-md-9 col-12">
                        <input type="text" class="input_focus addofferinput" name="account_number" required/>
                        <label class="float_label">{{__('site.account_number')}}</label>
                    </div>
                    <div class="form_label col-md-9 col-12">
                        <input type="text" class="input_focus addofferinput" name="iban" required/>
