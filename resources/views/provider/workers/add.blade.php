...
@section('title')
    {{__('site.add_new_worker')}}
@endsection
@section('content')
    @include('provider.layouts.nav')
    <div class="page-contant mt-4">
        <div class="container-fluid overflow-hidden">
            <h5 class="titlePages"> {{__('site.add_new_worker')}}</h5>
            <div class="add-form fadedown2">
                <form class="row flex-column align-items-center add-worker-form" method="POST" enctype="multipart/form-data" action="{{url('provider/add-worker')}}">
                    @csrf
                    <div class="form_label col-md-9 col-12">
                        <input type="text" class="input_focus addofferinput" name="name" required/>
                        <label class="float_label">{{__('site.worker_name')}}</label>
                    </div>
                    <div class="form_label col-md-9 col-12 position-relative">
                        <input id="phone" type="number" class="input_focus addofferinput" name="phone" required/>
                    ...
                    <div class="form_label col-md-9 col-12">
                        <input type="number" class="input_focus addofferinput" name="identity" required/>
                        <label class="float_label">{{__('site.identity')}}</label>
                    </div>
                    <div class="form_label col-md-9 col-12">
                        <input type="number" class="input_focus addofferinput" name="salary" required/>
...
@section('js')
    <script>
        $(document).ready(function(){
            $(document).on('submit','.add-worker-form',function(e){
                e.preventDefault();
